 ...
 * @property TransaksiKomisi[] $catatanBarangs
 */
class DataAgen extends \yii\db\ActiveRecord
{
...
public static function getAgenRefId(){
   ...
   public function getCatatanBarangs($no_acak)
    {
        return $this->hasMany(CatatanBarang::className(), ['id_data_agen' => 'id','no_acak'=>$no_acak]);
    }
    ...
    public static function cekIdAgenExists($no_acak){
      ...
      //  return var_dump($id_ref_agen);
        return \yii\helpers\ArrayHelper::map(\backend\models\DataAgen::find()->where(['id_ref_agen'=>$id_ref_agen])
                ->andWhere('id !=194')
                ->asArray()->all(),'no_acak','no_acak');
    }
}
